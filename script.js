...
createPopUp(clickedPoint);
  ...
  for (const store of data.features) {
    ...
    link.id = `link-${store.properties.idparc}`;
...
data1 = 'https://data.explore.star.fr/api/explore/v2.1/catalog/datasets/tco-metro-topologie-parcours-td/exports/geojson?lang=fr&timezone=Europe%2FBerlin';  
  jQuery.when(
    ...
    { 'id': 'tram',
      ...
      'paint' : {'line-color' : ['match', ['get', 'nomcourtligne'],
                                           'a','#ee1d23',
                                           'b','#00893e',
                                            '#000000'],
                 'line-width' : 4}
...
function createPopUp(currentFeature) {
  ...
  const popup = new maplibregl.Popup({ closeOnClick: false })
    ...
    .setHTML(`<h3>${currentFeature.properties.nom}</h3><h4>${currentFeature.properties.capaciteparking} places théoriques </h4>` + `<h4>${currentFeature.properties.jrdinfosoliste} places disponibles </h4>` + `<h4>${currentFeature.properties.jrdinfoelectrique} bornes éléctriques disponibles </h4>` )
    .addTo(map);
}
